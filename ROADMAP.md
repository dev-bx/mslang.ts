...
### Milestone 11 — Новые возможности языка
| id | Заголовок | Приоритет | Effort | Репо первым | Зависимости | Критерий готовности |
|---|---|---|---|---|---|---|
| M11-1 | F-1 стрелочные функции (`=>`) — лексер, парсер, интерпретатор | P2 | L | PHP | — | см. «8. Новые возможности языка», F-1 |
...
## 8. Новые возможности языка (фичи, которых нет ни в PHP, ни в TS)

В отличие от разделов P0–P2 выше (там устранялись расхождения между уже существующими реализациями), сюда попадают возможности языка, которых пока нет ни в PHP-эталоне, ни в TS-зеркале. Правило №0 действует и здесь без исключений: фича делается **сначала в PHP** (лексер → парсер → интерпретатор), и только потом переносится в TS один-в-один, с зеркальными тестами под одинаковыми именами в `php/tests/Test.php` и `tests/tests.ts`.

### F-1. Стрелочные функции (`(x) => x * 2`)

- **Статус:** не реализовано ни в PHP, ни в TS. Сейчас функция-значение доступна только через `function(x) { return x * 2; }` (раздел «Возможности языка» в `CLAUDE.md` обоих репозиториев); токен `=>` лексер не распознаёт.
- **Зачем:** короткая запись колбэков для массивных методов высшего порядка — `.filter()`/`.map()`/`.reduce()`/`.find()`/… (`php/src/Interpreter.php:378-566`) — без многословного `function(x) { return ...; }`.
- **Что нужно сделать (PHP — первым):**
  - **Лексер** (`CodeLexer.php` → `lexer.ts`): новый токен `=>` (вид `LexerType`, числовое значение синхронизировать в TS).
  - **Парсер** (`CodeParser.php` → `parser.ts`): разбор параметров — голый идентификатор без скобок (`x => ...`) и список в скобках (`(a, b) => ...`, `() => ...`); тело — одно выражение с неявным `return` либо блок `{ ... }` с явными `return`.
  - **`this` внутри стрелочной функции — отдельная развилка, решение владельца.** В MSLang уже есть классы/`super`/собственный `this` у обычных функций; нужно решить — лексическое связывание `this` (как в JS, берёт `this` из окружающего кода) или собственный `this`, как у `function`. Без явного решения не начинать парсер/интерпретатор.
  - **Интерпретатор:** переиспользовать существующий путь значения функции (`StackVariableUserFunction` / `invokeUserFunction`), не заводить отдельный класс значения только из-за другого синтаксиса объявления.
  - Если потребуется новый `NodeType`/`InterpreterNodeType` — числовые значения констант обязаны быть согласованы между PHP и TS бит-в-бит (`npm run test:mirror`).
- **Готово, когда:** `[1,2,3].filter(x => x % 2 == 0)` и многоарная форма `(a, b) => a + b` разбираются и выполняются одинаково в обоих движках; зеркальные тесты добавлены в `php/tests/Test.php` и `tests/tests.ts` под одинаковыми именами; `npm run test:mirror` зелёный.
